...
import {
    getDateTimeFormat, getResponsiveSize, isCloseToBottom,
    isCloseToTop
} from '../../utils/AppUtils';
...
const ChatClientDetailScreen = (props) => {

    const { user, logout } = useContext(AuthenticationContext);
    const { socket, chatInfo, setChatInfo,
        agentInfo, setAgentInfo } = useContext(SocketConext);
    ...
    const onLoadListData = (isLoadMore = false) => {
        ...
        ChatService.getChatHistory(sData)
            .then(async (res) => {
                ...
                if (isLoadMore) {
                    list = [...chatInfo.detailInfo.list, ...list];
    ...
    const getSendInfo = () => {
        ...
        if (user?.role === 'client') {
            ...
            item.agent_id = agentInfo?.id
        } else {
            item.agent_id = user?.id;
            item.client_id = agentInfo?.id
        }
    ...
    return (
        <LayoutMainView isAvoid={true}>
                ...
                <View style={[CommonStyles.flex_row, {
                    ...
                    <View style={[CommonStyles.flex_row, { flex: 1 }]}>
                        <View style={{ paddingRight: getResponsiveSize(6) }}>
                            <ChatUserAvatar picture={agentInfo?.avatar}
                                style={{ marginRight: getResponsiveSize(10) }}
                                name={agentInfo?.name}
                                mobileStatus={agentInfo?.mobile_status}
                            />
                        </View>

                        <View style={{ flex: 1 }}>
                            <Text numberOfLines={1} style={[CommonStyles.main_font_medium, { color: Theme.colors.primaryColorDark, fontSize: getResponsiveSize(18) }]}>{agentInfo?.name}</Text>
                            <View style={[CommonStyles.flex_row, { marginTop: -4 }]}>
                                <Text numberOfLines={1} style={[CommonStyles.main_font_regular, { marginTop: getResponsiveSize(4), fontSize: getResponsiveSize(12), color: Colors.grey, flex: 1 }]}>{getStatusTitle(agentInfo?.mobile_status)}</Text>
                            </View>
                        </View>
                        {
            ...
            <View style={[styles.bottomControlContainer, CommonStyles.flex_row, {
                ...
                {
                    detailInfo?.isTyping &&
                    <Text style={[CommonStyles.main_font_regular, styles.typingFlagWrapper]}>{agentInfo?.name + " " + getString('is typing...')}</Text>
                }
...
export default ChatClientDetailScreen;
